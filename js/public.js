...
function matchCardHTML(p){
  ...
  return `<div class="match-card${isLive?' live':''}${+p.totalEnSerie>1?' serie-mark':''}">
    <div class="mh"><div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap">${stateBadge}${serieTag}<span>${cat?.nombre||'—'}</span></div><span>${p.fecha?fmtDate(p.fecha)+(p.hora?' · '+p.hora:''):'Fecha por confirmar'}</span></div>
    <div class="mb">
    ...
    <div class="mf"><span>🟨 ${arb?.nombre||'Árbitro pendiente'}</span><span style="color:var(--g2)">${local?.seccion||''} vs ${visit?.seccion||''}</span></div>
  </div>`;
}
