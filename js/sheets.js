...
const API = {
  async call(action, data) {
    ...
    // Siempre GET — así GAS no hace preflight CORS
...
const DB = {
  ...
  getUsuarios()              { return this.getSheet('usuarios'); },
  getUsuario(id)             { return this.getUsuarios().find(u => u.id === id); },
  getUsuarioByCredentials(u,p) {
    return this.getUsuarios().find(x =>
      x.usuario === u && x.password === p && (x.estado === 'activo' || !x.estado)
    );
  },
