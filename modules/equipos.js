...
const Equipos = {
  ...
  render() {
    ...
    const html = `<div class="fade-in">
      <div class="section-header">
        ...
        ${Auth.isAdmin()?`<button class="btn btn-primary" onclick="Equipos.formNuevo()">+ Registrar Equipo</button>`:''}
      </div>
    ...
    Modal.open(`
      ...
      <div class="modal-body">
        <div class="form-grid">
          ...
          <div class="input-group"><label>Sección *</label>
            <input class="form-input" id="eq-sec" value="${eq?.seccion||''}" placeholder="Ej: 3A, 5B..." /></div>
  ...
  async guardar(id) {
    ...
    if (!cat||!sec||!nombre) { Toast.error('Categoría, sección y nombre son obligatorios.'); return; }
    try {
