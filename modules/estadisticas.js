...
const Usuarios = {
  render() {
    ...
    const html = `<div class="fade-in">
      <div class="section-header">
        <div><div class="section-title">USUARIOS</div><div class="section-sub">Gestión de accesos</div></div>
        ${Auth.isAdmin()?`<button class="btn btn-primary" onclick="Usuarios.formNuevo()">+ Nuevo Usuario</button>`:''}
      </div>
      <div class="card" style="padding:20px">
        <div style="background:rgba(255,215,0,0.06);border:1px solid rgba(255,215,0,0.18);border-radius:10px;padding:16px;margin-bottom:18px;font-size:13px;color:var(--gray)">
          🔒 Por seguridad, la lista de usuarios no se carga en el navegador.
          Para gestionar usuarios usa el menú <strong style="color:var(--white)">⚽ Intramuros 2026 → Cambiar contraseña</strong> en Google Sheets,
          o crea/edita usuarios con el botón de abajo.
        </div>
        <div style="display:flex;align-items:center;gap:14px;padding:14px;background:var(--black4);border-radius:10px">
          <div style="width:40px;height:40px;background:var(--yellow);border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:18px">👑</div>
          <div>
            <div style="font-weight:700">${session?.nombre||'—'}</div>
            <div style="font-size:12px;color:var(--gray)">@${session?.usuario||'—'} · ${session?.rol||'—'} · Sesión actual</div>
          </div>
        </div>
        ${Auth.isAdmin()?`
        <div style="margin-top:14px;display:flex;gap:10px;flex-wrap:wrap">
          <button class="btn btn-secondary" onclick="Usuarios.cambiarMiPassword()">🔑 Cambiar mi contraseña</button>
          <button class="btn btn-primary" onclick="Usuarios.formNuevo()">+ Crear usuario</button>
        </div>`:'<div style="margin-top:14px"><button class="btn btn-secondary" onclick="Usuarios.cambiarMiPassword()">🔑 Cambiar mi contraseña</button></div>'}
      </div>
    </div>`;
    document.getElementById('content-area').innerHTML = html;
  },
  ...
  async guardar(id) {
    ...
    // Hashear contraseña ANTES de enviar — nunca en texto plano
    if (pass) data.password = pass; // GAS hashea con SHA-256 al recibir
    try {
  ...
  async cambiarMiPassword() {
    const session = Auth.getSession();
    Modal.open(
      '<div class="modal-header"><span class="modal-title">🔑 CAMBIAR CONTRASEÑA</span><button class="modal-close" onclick="Modal.close()">✕</button></div>' +
      '<div class="modal-body">' +
        '<div class="input-group"><label>Nueva contraseña *</label>' +
          '<input class="form-input" type="password" id="cp-pass1" placeholder="••••••••" /></div>' +
        '<div class="input-group"><label>Confirmar contraseña *</label>' +
          '<input class="form-input" type="password" id="cp-pass2" placeholder="••••••••" /></div>' +
      '</div>' +
      '<div class="modal-footer">' +
        '<button class="btn btn-secondary" onclick="Modal.close()">Cancelar</button>' +
        '<button class="btn btn-primary" onclick="Usuarios._guardarPassword()">💾 Cambiar</button>' +
      '</div>', 'modal-sm');
  },

  async _guardarPassword() {
    const p1 = document.getElementById('cp-pass1').value;
    const p2 = document.getElementById('cp-pass2').value;
    if (!p1) { Toast.error('Ingresa la nueva contraseña.'); return; }
    if (p1 !== p2) { Toast.error('Las contraseñas no coinciden.'); return; }
    if (p1.length < 6) { Toast.error('Mínimo 6 caracteres.'); return; }
    const session = Auth.getSession();
    try {
      // Hashear antes de enviar
      const passwordHash = await hashPassword(p1);
      await DB.saveUsuario({ id: session.id, passwordHash, usuario: session.usuario, nombre: session.nombre, rol: session.rol, estado: 'activo' });
  ...
  eliminar(id) {
    UI.confirmDialog('¿Eliminar este usuario?', async () => {
