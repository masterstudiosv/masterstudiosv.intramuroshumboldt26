  ...
  _matchCard(p, inSerie = false) {
    ...
    return `<div class="match-card ${isLive?'live':''}" style="margin-bottom:${inSerie?'8':'12'}px">
      <div class="match-header">
        ...
        <span style="font-size:12px;color:var(--gray)">${p.fecha ? UI.formatDate(p.fecha)+' · '+p.hora : 'Fecha pendiente'}</span>
      </div>
    ...
    Modal.open(`
      ...
      <div class="modal-body">
        <div style="background:var(--black4);border-radius:10px;padding:20px;text-align:center;margin-bottom:20px">
          <div style="font-size:12px;color:var(--gray);letter-spacing:2px;margin-bottom:10px">${cat?.nombre||'—'} · ${p.fecha?UI.formatDate(p.fecha)+' '+p.hora:'Fecha pendiente'}</div>
